interface Facilities {
  ...
  export const addHotelOwnerProfile = async (profileData: HotelProfileCreate): Promise<HotelProfile> => {
    ...
    try {
      const response = await fetch("https://api.medebna.com/hotel-owner/add-hotel-owner-profile", {
        method: 'POST',
        headers: {
  ...
  export const updateHotelOwnerProfile = async (id: string, profileData: HotelProfile) => {
    const token = getToken();
    try {
      const response = await fetch(`https://api.medebna.com/hotel-owner/update-hotel-owner-profile/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
