...
export const addCarOwnerProfile = async (profileData: CarOwnerProfileCreate) => {
  ...
  try {
    const response = await fetch("http://194.5.159.228:5003/car-owner/add-car-owner-profile", {
      method: "POST",
      headers: {
...
export const fetchCarOwnerProfile = async (id: string): Promise<CarOwnerProfile | null> => {
  const token = getToken();
  try {
    const response = await fetch(`http://194.5.159.228:5003/car-owner/car-owner-detail/${id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
...
export const updateCarOwnerProfile = async (id: string, profileData: CarOwnerProfile): Promise<CarOwnerProfile> => {
  const token = getToken();
  try {
    const response = await fetch(`http://194.5.159.228:5003/car-owner/update-car-owner-profile/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
